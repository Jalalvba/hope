/**
 * The clinical system instruction — the whole of it, and nothing else.
 *
 * This file is deliberately just a header, an import, and one long template
 * literal, so the prompt can be read straight through as prose or handed to
 * another model for review without wading through application logic. Keep it
 * that way: no helpers, no conditionals, no assembly code here.
 *
 * Sent via Gemini's native `systemInstruction` field (see lib/ai/gemini.ts),
 * not concatenated into the user prompt — that keeps the persona and clinical
 * profile out of the conversational turn itself. Both generate routes send
 * this same text; see lib/ai/analyzePrompt.ts for how it is dispatched.
 *
 * Contains the user's real identity, history, and schema architecture. Do not
 * templatize or genericize it.
 */
